...
_DISCOVERY_GROUP_RETRYABLE_STATUSES = {404, 408, 409, 425, 429, 500, 502, 503, 504}
...
def configure_logging(level: str | int | None = None, *, force: bool = False) -> None:
    ...
    if isinstance(level, str):
        ...
        lvl = int(level)
    logging.basicConfig(
        format=_DEFAULT_LOG_FORMAT, datefmt=_DEFAULT_LOG_DATEFMT, level=lvl, force=force
    )
...
def _extract_last_status_code(exc) -> int | None:
    """
    Best-effort parse of `last_status` codes from helper exception text.
    """
    match = re.search(r"last_status:\s*([0-9]{3})", str(exc), flags=re.IGNORECASE)
    if not match:
        return None
    try:
...
class Workspaces:
    _state_map = requests.structures.CaseInsensitiveDict(
        {
            ...
            "MonitorOnly": "associatedPartner",
            "RequiresInvestigation": "candidateInvestigate",
            "Dismissed": "dismissed",
        }
    )
    _easm_regions = [
    ...
    _metrics = [
        ...
        "site_status_requires_authorization",
        "site_status_broken",
        "site_status_browser_error",
        "site_status_broken_cert_issue",
        "site_status_active_cert_issue",
        "site_status_requires_authorization_cert_issue",
        "site_status_browser_error_cert_issue",
        "pii_https",
        "pii_http",
        "pii_ssl_posture_md5",
        "pii_ssl_posture_sha1",
        "pii_ssl_posture_sha256",
        "pii_ssl_posture_other",
        "pii_ssl_posture_nocert",
        "login_https",
        "login_http",
        "login_ssl_posture_md5",
        "login_ssl_posture_sha1",
        "login_ssl_posture_sha256",
        "login_ssl_posture_other",
        "login_ssl_posture_nocert",
        "first_party_cookie_violation_https",
        "first_party_cookie_violation_http",
        "third_party_cookie_violation_https",
        "third_party_cookie_violation_http",
    ]
    _label_colors = [
        "red",
        "green",
    ...
    _facet_filters = {
        "assetSecurityPolicies": ("policyName", "description"),
        "attributes": ("attributeType", "attributeValue"),
        "banners": ("banner", "port"),
        # Single-element tuples must include a trailing comma; otherwise Python treats them as strings.
        "cookies": ("cookieName",),
        "finalIpBlocks": ("ipBlock",),
        "headers": ("headerName", "headerValue"),
        "ipBlocks": ("ipBlock",),
        "location": ("value,countrycode", "value,countryname", "value,latitude", "value,longitude"),
        "reputations": ("threatType", "listName"),
        "resourceUrls": ("url",),
        "responseHeaders": ("headerName", "headerValue"),
        "services": ("port", "scheme", "portStates,value"),
        "soaRecords": ("nameServer", "email", "serialNumber"),
        "sslServerConfig": ("cipherSuites", "tlsVersions"),
        "webComponents": ("name", "type", "version"),
        "cveId": ("webComponent", "name", "cvssScore"),
    }
    ...
    def __validate_asset_id__(self, asset_id):
        match = re.match(
            r"(as\$\$\S+|contact\$\$\S+|domain\$\$\S+|host\$\$\S+|ipAddress\$\$\S+|ipBlock\$\$\S+|page\$\$\S+|sslCert\$\$\S+)",
            asset_id,
        )
        if match:
        ...
        if self.__verify_workspace__(workspace_name):
            ...
            if disco_name:
                ...
                r = self.__workspace_query_helper__(
                    "__get_discovery_group_runs__",
                    method="get",
                    endpoint=f"discoGroups/{disco_name}/runs",
                    workspace_name=workspace_name,
                )
                for run in _response_items(r.json()):
                    disco_results[disco_name].append(
                        {
                            "state": run["state"],
                            "submittedDate": run["submittedDate"],
                            "completedDate": run["completedDate"],
                            "totalAssetsFoundCount": run["totalAssetsFoundCount"],
                        }
                    )
            else:
                disco_names = list(
                    set(
                        [
                            run["name"]
                            for run in _response_items(
                                self.get_discovery_groups(workspace_name, noprint=True)
                            )
                        ]
                    )
                )
                for disco in disco_names:
                    ...
                    r = self.__workspace_query_helper__(
                        "__get_discovery_group_runs__",
                        method="get",
                        endpoint=f"discoGroups/{disco}/runs",
                        workspace_name=workspace_name,
                    )
                    for run in _response_items(r.json()):
                        disco_results[disco].append(
                            {
                                "state": run["state"],
                                "submittedDate": run["submittedDate"],
                                "completedDate": run["completedDate"],
        ...
        def __nested_filter_creator__(asset, attribute_name=""):
            ...
            for key, val in vars(asset).items():
                ...
                if isinstance(getattr(asset, key), (str, int, bool, float)):
                    ...
                    for list_item in getattr(asset, key):
                        if isinstance(list_item, str):
                            ...
                            try:
                                ...
                                getattr(self.filters, key)[tuple([list_item])]["assets"].append(
                                    asset.id
                                )

                            except KeyError:
                                getattr(self.filters, key)[tuple([list_item])] = {
                                    "count": 1,
                                    "assets": [asset.id],
                                }
                            ...
                            for cert_key, cert_val in list_item.items():
                                if isinstance(cert_val, list):
                                    ...
                                    for sub_cert_val in cert_val:
                                        try:
                                            getattr(self.filters, cert_key)[tuple([sub_cert_val])][
                                                "count"
                                            ] += 1
                                            getattr(self.filters, cert_key)[tuple([sub_cert_val])][
                                                "assets"
                                            ...
                                            getattr(self.filters, cert_key)[
                                                tuple([sub_cert_val])
                                            ] = {"count": 1, "assets": [asset.id]}

                                    if attribute_name == key:
                                        break

                elif (
                    isinstance(getattr(asset, key), list)
                    and not ssl_cert
                    and key not in self._facet_filters
                ):
                    for list_item in getattr(asset, key):
                        if isinstance(list_item, str):
                            if attribute_name and not attribute_name == key:
                            ...
                            try:
                                ...
                                getattr(self.filters, key)[(list_item.get("value"),)][
                                    "assets"
                                ].append(asset.id)
                    ...
                    isinstance(getattr(asset, key), list)
                    and not ssl_cert
                    and key in self._facet_filters
                ):
                    if attribute_name and not attribute_name == key:
                        continue
                    for list_item in getattr(asset, key):
                        ...
                        if key == "sslServerConfig":
                            ...
                            if tls_versions:
                                for idx, _tlsval in enumerate(tls_versions):
                                    cipher = (
                                        cipher_suites[idx] if idx < len(cipher_suites) else None
                                    )
                                    tls = tls_versions[idx]
                                    __inc_filter__(key, (cipher, tls))
                            else:
                                logging.debug(
                                    f"empty sslServerConfig[tlsVersions] list in asset {asset.id}"
                                )
                                pass
                            ...
                            if port_states:
                                for port_state in port_states:
                                    __inc_filter__(
                                        ...
                                        (
                                            list_item.get("port"),
                                            list_item.get("scheme"),
                                            port_state.get("value"),
                                        ),
                                    )
                            else:
                                __inc_filter__(
                                    key, (list_item.get("port"), list_item.get("scheme"), None)
                                )
            ...
            if len(val) == 0:
                ...
                for sub_key, sub_val in val.items():
                    ...
                    if not sub_val["count"] == dedup_count:
                        ...
                        getattr(self.filters, key)[sub_key] = {
                            "count": dedup_count,
                            "assets": deduped,
                        }
    ...
    def __workspace_query_helper__(
        self,
        calling_func,
        method,
        endpoint,
        ...
        if url:
            ...
            helper_url = (
                f"https://{self._workspaces[workspace_name][0]}/{urllib.parse.quote(endpoint)}"
            )
        elif workspace_name and not data_plane:
            helper_url = (
                f"https://{self._workspaces[workspace_name][1]}/{urllib.parse.quote(endpoint)}"
            )
        ...
        if workspace_name:
            for workspace in r.json()["value"]:
                if workspace["name"].lower() == workspace_name.lower():
                    self._workspaces[workspace["name"]] = (
                        f"{workspace['properties']['dataPlaneEndpoint']}{workspace['id'].replace('/providers/Microsoft.Easm', '')}",
    ...
    def create_workspace(self, resource_group_name=None, region=None, workspace_name=None):
        ...
        if self.__verify_workspace__(workspace_name):
            ...
            self._workspaces[workspace_name] = (
                f"{r.json()['properties']['dataPlaneEndpoint']}{r.json()['id'].replace('/providers/Microsoft.Easm', '')}",
                f"management.azure.com{r.json()['id']}",
            )
    ...
    def get_discovery_templates(self, org_name, workspace_name="", **kwargs):
        ...
        if self.__verify_workspace__(workspace_name):
            ...
            r = self.__workspace_query_helper__(
                "get_discovery_templates",
                method="get",
                endpoint="discoTemplates",
                params=params,
                workspace_name=workspace_name,
            )
            templates = []
    ...
    def __get_discovery_group_runs_with_retry__(
        self, disco_name, workspace_name="", max_attempts=3, backoff_max_s=5
    ):
        attempts = max(int(max_attempts or 1), 1)
        backoff_cap = max(float(backoff_max_s or 0), 0.0)
        last_exc = None

        for attempt in range(1, attempts + 1):
            try:
                return self.__get_discovery_group_runs__(
                    disco_name=disco_name, workspace_name=workspace_name
                )
            except Exception as exc:
                last_exc = exc
                status = _extract_last_status_code(exc)
                retryable = status is None or status in _DISCOVERY_GROUP_RETRYABLE_STATUSES
                if not retryable or attempt >= attempts:
                    raise
                sleep_s = min(2 ** (attempt - 1), backoff_cap)
                if sleep_s > 0:
                    time.sleep(sleep_s)
    ...
    def create_discovery_group(self, disco_template="", disco_custom=None, workspace_name="", **kwargs):
        ...
        if not disco_template and not disco_custom:
            ...
            if self.__verify_workspace__(workspace_name):
                ...
                self.__workspace_query_helper__(
                    ...
                    method="put",
                    endpoint=f"discoGroups/{disco_name}",
                    payload=payload,
                    workspace_name=workspace_name,
                )
    ...
    def get_discovery_groups(
        ...
        if self.__verify_workspace__(workspace_name):
            ...
            if max_page_size:
                params["maxpagesize"] = max(int(max_page_size), 1)
            r = self.__workspace_query_helper__(
                "get_discovery_groups",
                method="get",
                endpoint="discoGroups",
                params=params,
                workspace_name=workspace_name,
            )
            payload = r.json()
    ...
    def delete_discovery_group(self, name, workspace_name="", **kwargs):
        if not workspace_name:
            workspace_name = self._default_workspace_name
        if not self.__verify_workspace__(workspace_name):
            self.__raise_workspace_not_found__(workspace_name)
        ...
        out = {
            ...
            "status": status,
            "verifiedDeleted": None,
        }

        if kwargs.get("verify_deleted"):
            verify_attempts = max(int(kwargs.get("verify_max_retry", 3) or 1), 1)
            verify_backoff_max_s = max(float(kwargs.get("verify_backoff_max_s", 5) or 0), 0.0)
            deleted = False
            for attempt in range(1, verify_attempts + 1):
                try:
                    payload = self.get_discovery_groups(
                        workspace_name=workspace_name,
                        skip=0,
                        max_page_size=100,
                        noprint=True,
                    ...
                    if disco_name.lower() not in names:
                        deleted = True
                        break
                except Exception as exc:
                    status_code = _extract_last_status_code(exc)
                    retryable = (
                        status_code is None
                        or status_code in _DISCOVERY_GROUP_RETRYABLE_STATUSES
                    )
                    if not retryable:
                        raise
                if attempt < verify_attempts:
                    sleep_s = min(2 ** (attempt - 1), verify_backoff_max_s)
                    if sleep_s > 0:
                        time.sleep(sleep_s)
            out["verifiedDeleted"] = deleted
    ...
    def get_workspace_assets(
        ...
        max_page_count=0,
        get_all=False,
        auto_create_facet_filters=True,
        get_recent=True,
        last_seen_days_back=30,
        date_range_start="",
        date_range_end="",
        ...
        **kwargs,
        ...
        if self.__verify_workspace__(workspace_name):
            if asset_list_name and not hasattr(self, asset_list_name):
                ...
                asset_list_name
                and hasattr(self, asset_list_name)
                and isinstance(getattr(self, asset_list_name), AssetList)
            ):
                pass

            if max_page_size > 100:
                logging.warning(
                    "max_page_size cannot be greater than 100, setting max_page_size=100"
                )
                max_page_size = 100
            ...
            while run_query:
                ...
                if max_assets and len(getattr(self, asset_list_name).assets) >= max_assets:
                    ...
                    getattr(self, asset_list_name).assets = getattr(self, asset_list_name).assets[
                        :max_assets
                    ]
                    run_query = False
    ...
    def get_workspace_asset_by_id(
        ...
        if self.__verify_workspace__(workspace_name):
            ...
            r = self.__workspace_query_helper__(
                "get_workspace_asset_by_id",
                method="get",
                endpoint=f"assets/{verified_asset_id}",
                workspace_name=workspace_name,
            )

            self.__asset_content_helper__(
                r,
                asset_id=orig_asset_id,
                get_recent=get_recent,
                last_seen_days_back=last_seen_days_back,
                date_range_start=date_range_start,
                date_range_end=date_range_end,
        ...
        **kwargs,
        ...
        if self.__verify_workspace__(workspace_name):
            if severity.lower() == "high":
                ...
                logging.warning(
                    f"supplied value of severity='{severity}' not one of 'high','medium','low'; defaulting to all severities"
                )
            ...
            r_summarize = self.__workspace_query_helper__(
                "get_workspace_risk_observations",
                method="post",
                endpoint="reports/assets:summarize",
            ...
            if not metrics:
                ...
                for key, val in metrics.items():
                    ...
                    get_next = True
                    while get_next:
                        r_snapshot = self.__workspace_query_helper__(
                            "get_workspace_risk_observations",
                            method="post",
                            endpoint="reports/assets:snapshot",
    ...
    def query_facet_filter(
        ...
        if facet_filter:
            for facet_key, facet_val in sorted(
                getattr(self.filters, facet_filter).items(),
        ...
        **kwargs,
        ...
        if self.__verify_workspace__(workspace_name):
            try:
                ...
                asset_count_r = self.__workspace_query_helper__(
                    "update_asset_state",
                    method="post",
                    endpoint="reports/assets:summarize",
                    payload=asset_count_payload,
                )
    ...
    def get_workspace_asset_summaries(
        ...
        **kwargs,
        ...
        if (
            (not query_filters and not metric_categories and not metrics)
            or (query_filters and metric_categories and metrics)
            or (query_filters and metric_categories and not metrics)
            or (query_filters and not metric_categories and metrics)
            or (not query_filters and metric_categories and metrics)
        ):
            logging.error(
        ...
        if metric_categories and (
            not isinstance(metric_categories, list)
            or not all(met_cat in self._metric_categories for met_cat in metric_categories)
        ):
            logging.error(
        ...
        if self.__verify_workspace__(workspace_name):
            ...
            for idx, summary in enumerate(r.json()["assetSummaries"]):
                if (
                    (submitted[idx] == summary["metricCategory"])
                    or (submitted[idx] == summary["metric"])
                    or (submitted[idx] == summary["filter"])
                ):
                    self.asset_summaries[submitted[idx]] = {
                        "count": summary["count"],
...
class Asset:
    ...
    def __parse_workspace_assets__(
        self,
        asset_object,
        get_recent=True,
        last_seen_days_back=30,
            ...
            if key in self._exclude_attributes:
                ...
                for sub_key, sub_val in val.items():
                    if sub_key in self._exclude_attributes:
                            ...
                            try:
                                if (
                                    ...
                                    if (
                                        (
                                            get_recent
                                            and "recent" in sub_sub_val
                                            and sub_sub_val.get("recent") == True
                                        )
                                        or (
                                            "lastSeen" in sub_sub_val
                                            and parser.parse(sub_sub_val.get("lastSeen"))
                                            >= last_seen
                                        )
                                        or (
                                            date_range_start
                                            and date_range_end
                                            and "firstSeen" in sub_sub_val
                                            and "lastSeen" in sub_sub_val
                                            and date_start.astimezone(tz=datetime.timezone.utc)
                                            >= parser.parse(sub_sub_val.get("firstSeen"))
                                            and parser.parse(sub_sub_val.get("lastSeen"))
                                            >= date_end.astimezone(tz=datetime.timezone.utc)
                                        )
                                        or (
                                            date_range_start
                                            and not date_range_end
                                            and "lastSeen" in sub_sub_val
                                            and parser.parse(sub_sub_val.get("lastSeen"))
                                            >= date_start.astimezone(tz=datetime.timezone.utc)
                                        )
                                        or (
                                            date_range_end
                                            and not date_range_start
                                            and "lastSeen" in sub_sub_val
                                            and parser.parse(sub_sub_val.get("lastSeen"))
                                            <= date_end.astimezone(tz=datetime.timezone.utc)
                                        )
                                        ...
                                        new_val = {
                                            k: sub_sub_val[k]
                                            for k in set(list(sub_sub_val.keys()))
                                            - self._exclude_keys
                                        }
                                    ...
                                    new_val = {
                                        k: sub_sub_val[k]
                                        for k in set(list(sub_sub_val.keys())) - self._exclude_keys
                                    }
                        ...
                        logging.info(
                            f"unknown attribute type in {asset_object['id']} -- {sub_key}:{sub_val} -- {type(sub_key)}"
                        )
                        pass
        return self
