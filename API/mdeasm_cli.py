...
def _build_completion_index(parser: argparse.ArgumentParser) -> dict[str, dict[str, list[str]]]:
    """
    Build a command-path index for completion generation.

    Keys are command paths joined by spaces (root is ""), and values contain
    `subcommands` and `options` lists for that parser scope.
    """
    index: dict[str, dict[str, list[str]]] = {}

    def walk(cur_parser: argparse.ArgumentParser, path_parts: list[str]) -> None:
        path_key = " ".join(path_parts)
        subcommands: list[str] = []
        options: list[str] = []
        child_parsers: dict[str, argparse.ArgumentParser] = {}

        for action in cur_parser._actions:
            if action.option_strings:
                options.extend(action.option_strings)
            if isinstance(action, argparse._SubParsersAction):
                for name, sub_parser in sorted(action.choices.items()):
                    subcommands.append(name)
                    child_parsers[name] = sub_parser
...
def _render_completion_script(parser: argparse.ArgumentParser, *, shell: str) -> str:
    index = _build_completion_index(parser)

    lines: list[str] = [
        f"# Generated by `mdeasm completions {shell}`",
    ...
    lines.extend(
        [
            "declare -A _MDEASM_SUBCOMMANDS=(",
        ]
    )
    for path_key, payload in sorted(index.items()):
        value = " ".join(payload.get("subcommands") or [])
        lines.append(
            f'  ["{_escape_shell_double_quoted(path_key)}"]="{_escape_shell_double_quoted(value)}"'
        )
    lines.append(")")
    lines.append("declare -A _MDEASM_OPTIONS=(")
    for path_key, payload in sorted(index.items()):
        value = " ".join(payload.get("options") or [])
        lines.append(
            f'  ["{_escape_shell_double_quoted(path_key)}"]="{_escape_shell_double_quoted(value)}"'
        )
    lines.extend(
        [
            ...
            "",
            "_mdeasm_complete() {",
            "  local cur path subs opts candidates word i",
            '  cur="${COMP_WORDS[COMP_CWORD]}"',
            '  path=""',
            "  for ((i=1; i<COMP_CWORD; i++)); do",
            '    word="${COMP_WORDS[i]}"',
            '    [[ "$word" == -* ]] && continue',
            '    subs="${_MDEASM_SUBCOMMANDS[$path]}"',
            '    case " ${subs} " in',
            '      *" ${word} "*) path="${path:+$path }${word}" ;;',
            "    esac",
            "  done",
            '  subs="${_MDEASM_SUBCOMMANDS[$path]}"',
            '  opts="${_MDEASM_OPTIONS[$path]}"',
            '  if [[ "$cur" == -* ]]; then',
            '    candidates="$opts"',
            "  else",
            '    candidates="$subs $opts"',
            "  fi",
            '  COMPREPLY=( $(compgen -W "$candidates" -- "$cur") )',
            "}",
            "complete -o default -F _mdeasm_complete mdeasm",
            "",
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    completions = sub.add_parser(
        "completions",
        help="Generate shell completion scripts for mdeasm",
    )
    completions.add_argument(
        "shell",
        choices=["bash", "zsh"],
        help="Target shell completion format",
    )
    completions.add_argument("--out", default="", help="Output path (default: stdout)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "completions":
        try:
            ...
            script = _render_completion_script(build_parser(), shell=args.shell)
            if out_path is None:
                sys.stdout.write(script)
            else:
                _atomic_write_text(out_path, script, encoding="utf-8")
            return 0
        except Exception as e:
            sys.stderr.write(f"completions generation failed: {e}\n")
            return 1
